...
if (SERVER) then
	util.AddNetworkString("impulseTextList")
	util.AddNetworkString("impulseTextAdd")
	util.AddNetworkString("impulseTextRemove")

	impulse.Log.AddType("undo3dText", function(client)
	...
	language.Add("Undone_impulse3dText", "Removed 3D Text")

	function PLUGIN:GenerateMarkup(text)
		local object = impulse.markup.Parse("<font=impulse3D2DFont>"..text:gsub("\\n", "\n"))
	...
	net.Receive("impulseTextAdd", function()
	...
	net.Receive("impulseTextRemove", function()
		local index = net.ReadUInt(32)
	...
	net.Receive("impulseTextList", function()
		local length = net.ReadUInt(32)
		local data = net.ReadData(length)
		local uncompressed = util.Decompress(data)

		if (!uncompressed) then
			ErrorNoHalt("[impulse] Unable to decompress text data!\n")
			return
		end
		...
		for k, v in pairs(PLUGIN.list) do
			...
			local object = impulse.markup.Parse("<font=impulse3D2DFont>"..v[3]:gsub("\\n", "\n"))
	...
	function PLUGIN:HUDPaint()
		if (impulse.chat.currentCommand != "textremove") then
			return
		end

		local radius = tonumber(impulse.chat.currentArguments[1]) or 100

		surface.SetDrawColor(200, 30, 30)
		surface.SetTextColor(200, 30, 30)
		surface.SetFont("impulseMenuButtonFont")
	...
	function PLUGIN:PostDrawTranslucentRenderables(bDrawingDepth, bDrawingSkybox)
		...
		if (impulse.chat.currentCommand == "textadd") then
			local arguments = impulse.chat.currentArguments
...
impulse.RegisterCommand("TextAdd", {
	description = "@cmdTextAdd",
	adminOnly = true,
	arguments = {
		impulse.type.string,
		bit.bor(impulse.type.number, impulse.type.optional)
	},
	OnRun = function(self, client, text, scale)
		...
		undo.Create("impulse3dText")
			undo.SetPlayer(client)
			undo.AddFunction(function()
				if (PLUGIN:RemoveTextByID(index)) then
					impulse.Log.Add(client, "undo3dText")
				end
			end)
		undo.Finish()
...
impulse.RegisterCommand("TextRemove", {
	description = "@cmdTextRemove",
	adminOnly = true,
	arguments = bit.bor(impulse.type.number, impulse.type.optional),
